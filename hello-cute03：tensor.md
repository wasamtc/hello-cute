...
tensor这一节没有太多需要解释的东西，只要看懂上一节layout的代数运算应该没有问题，所以这里基本直接使用官方文档。其中比较重要的是tensor的分区(partitioning)，分区分为两步：1. composition 或 tiling（逻辑同divide）；2. slicing。
...
2. 取出切片坐标里那些由 `_` 对应的 layout 模式，重新构造一个新的 layout（注意stride是不变的）。这个新 layout 再与新 iterator 一起组成新的 tensor。
...
<img src="./cute原文档/images/slice.png" alt="多种切片得到的 subtensor 高亮示意图" />
...
<img src="./cute原文档/images/tv_layout.png" alt="线程-值布局（TV-layout）示意图" />
